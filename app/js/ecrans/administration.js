/* ============================================================
   Général › Administration (maquette 04-gAdmin.png)
   Onglets Équipes, Référentiels, Champs du formulaire.
   Lecture pour tous ; modification réservée aux administrateurs
   globaux (table administrateurs, contrôlée aussi par la base).
   ============================================================ */
'use strict';
...
Ecrans.administration = {
  ...
  section: 'general',
  rendre() {
    const u = ui('administration', { onglet: 'equipes', referentiel: 'type' });
    ...
    ], u.onglet, 'ongletAdministration');
    const corps = u.onglet === 'equipes' ? this.equipes() : u.onglet === 'referentiels' ? this.referentiels(u.referentiel) : this.champs();
    const retour = !etat.equipeCourante ? `<button class="btn" data-action="aller" data-ecran="choixEquipe">Choix de l’équipe</button>` : '';
    return `
    <div class="ecran">
      ${C.entete('Administration', 'Équipes, référentiels et champs du formulaire de demandes' + (etat.estAdmin ? ' · vous êtes administrateur' : ''), retour)}
      ${onglets}${corps}
    </div>`;
  },
  ...
  equipes() {
    ...
    const lignes = etat.d.equipes.map(e => {
      ...
      return `<tr><td><span class="ligne-flex">${C.pastille(e.couleur)}${esc(e.nom)} ${C.code(e.prefixe)}</span></td><td>${resp ? esc(resp.nom) : '<span class="pale">—</span>'}</td>
        ...
        <td class="num">${etat.estAdmin || estResponsableDe(e.id) ? `<a data-action="modifierEquipe" data-id="${e.id}">Modifier</a>` : ''}</td></tr>`;
    }).join('');
    return `<div class="carte"><div class="carte-titre"><h2>Équipes</h2>
        ${etat.estAdmin ? '<button class="btn primaire" data-action="nouvelleEquipe">+ Nouvelle équipe</button>' : ''}</div>
      <table class="tableau"><thead><tr><th>Équipe</th><th>Responsable</th><th>Membres</th><th>Projets</th><th></th></tr></thead>
      <tbody>${lignes || `<tr><td colspan="5">${C.vide('Aucune équipe. Un administrateur crée la première équipe.')}</td></tr>`}</tbody></table></div>`;
  },

  referentiels(refId) {
    const esc = C.esc, admin = etat.estAdmin, ref = parId('referentiels', refId) || etat.d.referentiels[0];
    const gauche = etat.d.referentiels.map(r => `<a class="menu-lien${r.id === ref.id ? ' actif-clair' : ''}" style="color:var(--texte);justify-content:space-between;${r.id === ref.id ? 'background:#F3F6FF' : ''}"
    ...
    const valeurs = valeursDe(ref.id, true).map((v, i) => `
      <tr><td class="pale">${i + 1}</td>
        <td>${admin && !v.systeme ? `<input class="champ" value="${esc(v.libelle)}" data-action-change="renommerValeur" data-id="${v.id}">` : esc(v.libelle)}
          ${v.systeme ? '<span class="pale" title="Utilisée par les calculs : ni renommage ni suppression"> · système</span>' : ''}</td>
        <td>${admin ? `<input type="color" class="couleur-choix" value="${v.couleur}" data-action-change="colorerValeur" data-id="${v.id}">` : C.pastille(v.couleur)}</td>
        <td><button class="interrupteur${v.actif ? ' actif' : ''}" ${admin ? `data-action="basculerValeur" data-id="${v.id}"` : 'disabled'}></button></td></tr>`).join('');
    return `<div style="display:grid;grid-template-columns:240px minmax(0,1fr);gap:16px;align-items:start">
      ...
      <div class="carte"><div class="carte-titre"><h2>${esc(ref.nom)}</h2></div>
        ...
        ${admin ? `<form class="ligne-flex" style="padding:12px 16px" data-action-envoi="ajouterValeur" data-ref="${ref.id}">
...
Object.assign(Actions, {
  ...
  choisirReferentiel: d => majUi('administration', { referentiel: d.id }),
  nouvelleEquipe: () => majEtat({ modale: { type: 'equipe', id: null } }),
  modifierEquipe: d => majEtat({ modale: { type: 'equipe', id: d.id } }),
