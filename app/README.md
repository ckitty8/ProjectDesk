...
Vercel publie la branche `main` sur **https://project-desk-sepia.vercel.app** ; `vercel.json` redirige `/` vers `/app/`.
...
2. Le déclarer **administrateur global** (une seule fois, dans l'éditeur SQL de la console Neon) :
   ...
   ```
3. Se reconnecter : l'administrateur arrive directement dans l'outil ; l'écran **Administration** permet de créer les équipes (chaque équipe est une
